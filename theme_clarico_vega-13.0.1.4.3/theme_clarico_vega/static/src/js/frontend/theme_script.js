...
odoo.define('theme_clarico_vega.theme_script', function(require) {
    ...
    $( "#_submitRegistration" ).click(function() {
          ...
          if(email == '' || name == '' || password == '' || vat == '' || phone == '' || company_type == '' || confirm_password == ''){
              $("#loginRegisterPopup .oe_signup_form_ept .te_error-success").replaceWith('<div class="te_error-success alert alert-danger">Todos los campos son obligatorios.</div>');
              return false;
          }
                    
          ajax.jsonRpc('/web/signup_custom', 'call', {'tipo': 'registro', 'login':email,'name':name,'password':password,'confirm_password':confirm_password, 'vat':vat,'phone':phone,'company_type':company_type,'redirect':'','token':''}).then(function(data) {
              if(!data.is_success){
                  ...
                  $("#loginRegisterPopup .oe_signup_form_ept .te_error-success").replaceWith("<div class='te_error-success alert alert-danger'>Gracias por registrarse. En breve nos pondremos en contacto con usted para activarle el acceso a nuestra tienda.</div>");
                  return false;
              }
          });
    ...
    sAnimations.registry.reset_password_popup = sAnimations.Class.extend({
        ...
        console.log("Accedo al registro");
                ...
                ajax.jsonRpc('/web/signup_custom', 'call', {'login':email,'name':name,'password':password,'confirm_password':confirm_password, 'vat':vat,'phone':phone,'company_type':company_type,'redirect':'','token':''}).then(function(data) {
                    if(!data.is_success){
                        $("#loginRegisterPopup .oe_signup_form_ept .te_error-success").replaceWith("<div class='te_error-success alert alert-danger'>" + data.error + "</div>");
                    } else {
            $("#loginRegisterPopup .oe_signup_form_ept .te_error-success").replaceWith("<div class='te_error-success alert alert-danger'>Gracias por registrarse. En breve nos pondremos en contacto con usted para activarle el acceso a nuestra tienda.</div>");
