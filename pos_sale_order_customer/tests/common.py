...
class TestPointOfSaleCommon(TransactionCase):
    ...
    def setUp(self):
        ...
        self.cash_payment_method = self.pos_config\
            .payment_method_ids.filtered(lambda pm: pm.name == 'Cash')

        self.bank_payment_method = self.pos_config\
            .payment_method_ids.filtered(lambda pm: pm.name == 'Bank')
        ...
        refund_rep_lines = (
            ...
            | account_tax_05_incl_chicago
            | account_tax_10_incl
        ).mapped('refund_repartition_line_ids')

        tax_received_account = self.company\
            .account_sale_tax_id.mapped(
        ...
        (
            invoice_rep_lines
            | refund_rep_lines
        ).write({'account_id': tax_received_account.id})
...
class TestPoSCommon(TransactionCase):
    def setUp(self):
        ...
        self.receivable_account = self.pos_manager\
            .partner_id.property_account_receivable_id

        r_account = self.env['account.account'].create({
            'name': 'R Account',
            'code': 'RCV01',
        ...
        self.company.account_default_pos_receivable_account_id = r_account.id

        self.tax_received_account = self.company\
            .account_sale_tax_id.mapped(
                'invoice_repartition_line_ids.account_id'
            )

        self.pos_receivable_account = self.company\
        ...
        self.other_currency = self.env.ref('base.EUR')\
            if self.company_currency == self.env.ref('base.USD')\
        ...
        self.other_customer = self.env['res.partner'].create({
            'name': 'Other Customer',
            'property_account_receivable_id': self.other_receivable_account.id
        })
    ...
    def _create_other_currency_config(self):
        (
            ...
            | self.company_currency.rate_ids
        ).unlink()
    ...
    def create_ui_order_data(
        self, product_quantity_pairs,
        customer=False, is_invoiced=False,
        ...
        fiscal_position = customer.property_account_position_id\
            if customer else default_fiscal_position

        def create_order_line(product, quantity):
            price_unit = self.pricelist.get_product_price(
                product,
                quantity,
                False
            )
            ...
            tax_ids = fiscal_position.map_tax(product.taxes_id)\
                if fiscal_position else product.taxes_id
        ...
        order_lines = [
            create_order_line(product, quantity)
            for product, quantity in product_quantity_pairs
        ]

        total_amount_incl = sum(
            line[2]['price_subtotal_incl']
        ...
        total_amount_base = sum(
            line[2]['price_subtotal']
            for line in order_lines
        )
    ...
    def open_new_session(self):
        ...
        self.bank_pm = self.pos_session.payment_method_ids.filtered(
            lambda pm: not pm.is_cash_count and not pm.split_transactions
        )[:1]

        self.cash_split_pm = self.pos_session.payment_method_ids.filtered(
            lambda pm: pm.is_cash_count and pm.split_transactions
        )[:1]

        self.bank_split_pm = self.pos_session.payment_method_ids.filtered(
            lambda pm: not pm.is_cash_count and pm.split_transactions
        )[:1]
