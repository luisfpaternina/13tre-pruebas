...
class OdooWebsiteFileUpload(http.Controller):
    ...
    @http.route('/shop/payment/comment/add', type='http', auth="public", website=True)
    def payment_comment_add(self, url=None, upload=None, **post):
        cr, uid, context = request.cr, request.uid, request.context

        order = request.website.sale_get_order()

        Order = request.env['sale.order'].sudo().search([('id', '=', order.id)])  # registry for the attachment table
        comentario = request.httprequest.values.get('comentario_pedido', False)
