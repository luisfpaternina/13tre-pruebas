...
class TestDeliveryCarrier(TransactionCase):
    ...
    def setUp(self):
        ...
        self.product_template = self.env['product.template'].search(
            [], limit=1)
        ...
        self.state = self.env['res.country.state'].search(
            [('l10n_co_divipola', '=', 19)])
        self.from_to = self.env['res.country.town'].search(
            [('l10n_co_divipola', '=', 19100)])
        ...
        self.delivery_carrier = self.delivery_carrier.create({
            ...
            'town_from_ids': [(4, self.from_to.id, 0)],
    ...
    def test_not_town_to(self):
        ...
        self.delivery_sale_order_cost.write({
            'town_id': town_to.id
        })
        delivery_wizard = Form(self.env[
            'choose.delivery.carrier'].with_context({
        ...
        choose_delivery_carrier = delivery_wizard.save()
        choose_delivery_carrier.button_confirm()
        self.env['account.move']._get_town_id()
        self.delivery_sale_order_cost._get_town_id()

    def test_prepare_invoice(self):
        result = self.delivery_sale_order_cost._prepare_invoice()
        self.assertEqual(result['town_id'].id,
                         self.delivery_sale_order_cost.town_id.id)

    def test_prepare_invoice_values(self):
        result = self.env[
            'sale.advance.payment.inv']._prepare_invoice_values(
                self.delivery_sale_order_cost, 'test', 8000,
                self.delivery_sale_order_cost.order_line)
