...
class AuthenticationController(http.Controller):
    ...
    @http.route(['/api/signup'], auth="none", type='http', methods=['POST'], csrf=_csrf)
    @tools.common.parse_exception
    @tools.common.ensure_database
    @tools.common.ensure_module()
    def auth_signup(self, **kw):
        values = {key: request.params.get(key, None) for key in
                  ('login', 'name', 'password', 'vat', 'phone', 'company_type')}
        ctx = request.session.context.copy()
        supported_lang_codes = [code for code, _ in request.env['res.lang'].get_installed()]
        lang = ctx.get('lang', '').split('_')[0]
        if lang in supported_lang_codes:
            values['lang'] = lang
        ResUsers = request.env['res.users'].sudo()
        #result = ResUsers.with_context(mail_create_nosubscribe=True).create(values)
        ResUsers.with_context(mail_create_nosubscribe=True, force_company=request.params.get('company_id', False)).signup(values)
        request.env.cr.commit()
        result = ResUsers.search([("login", "=", request.params.get("login", None))]).ids
