...
class TestAccountFollowup(TestAccountNoChartCommon):
    ...
    def test_05_followup_multicompany(self):
        ...
        self.assertEqual(len(lines[1]['columns']), 9)

        self.assertEqual(lines[1]['columns'][7]['name'], 'Total Due')
        self.assertEqual(lines[1]['columns'][8]['name'], formatLang(self.env, 30.00, currency_obj=currency))
        ...
        self.assertEqual(lines[1]['columns'][7]['name'], 'Total Due')
        self.assertEqual(lines[1]['columns'][8]['name'], formatLang(self.env, 60.00, currency_obj=currency))
...
class TestPayment(TransactionCase):
    ...
    def setUp(self):
        ...
        self.followup_class = self.env['account.followup.report']
    ...
    def test_followup_partner(self):
        options = {
            'partner_id': False
        }
        self.followup_class._get_lines(options)
    
    def test_followup_print(self):
        self.followup_class.with_context({
            'print_mode': True
        })._get_columns_name({})
        self.followup_class._get_columns_name({})