...
class TestBitsHrPayrollNews(TransactionCase):
    ...
    def test_compute_check_hour_extra(self):
        self.salary_rule_2 = self.hr_salary_rule.create({
            'name': "Test Rule",
            'code': 60,
            'sequence': 100,
        ...
        self.payroll_new_2 = self.hr_payroll_new.create({
            'name': "Test Novelty",
            'payroll_structure_id': self.payroll_structure.id,
        ...
        self.payroll_new_2._compute_check_hour_extra()

    def test_compute_check_hour_extra_no_code(self):
        self.payroll_new._compute_check_hour_extra()

    def test_calculate_hours_time(self):
        self.payroll_new.calculate_hours_time()

    
    def test_calculate_hours_time_datetimestart_higher(self):
    ...
    def test_calculate_hours_time_datetimestart(self):
        self.payroll_new.write({
            'datetime_start': datetime.now(),
            'datetime_end': datetime.now()+timedelta(days=365),
    ...
    def test_calculate_hours_time_datetimestart_return(self):
        self.payroll_new.write({
            'datetime_end': datetime.now(),
            'datetime_start': datetime.now()+timedelta(days=365),
            'employee_payroll_news_ids': False
        })
        with self.assertRaises(UserError): 
            self.payroll_new.calculate_hours_time()
    
    def test_calculate_hours_time_datetimestart_not_employee(self):
        self.payroll_new.write({
            'datetime_start': datetime.now(),
            'datetime_end': datetime.now()+timedelta(days=365),
            'employee_payroll_news_ids': False
