...
class ReportAccountAgedPartner(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        result = super(ReportAccountAgedPartner,
    ...
    @api.model
    def _get_partner_lines(self, results, amls, sign, options):
        lines = []
        for values in results:
            ...
            lines.append(vals)
            if ('partner_%s' % (values['partner_id'],)
                    in options.get('unfolded_lines')):
                for line in amls[values['partner_id']]:
                    ...
                    caret_type = ('account.invoice.out'
                                  if aml.move_id.is_sale_document()
                                  else caret_type)
                    caret_type = ('account.payment'
                                  if aml.payment_id else caret_type)
                    ...
                    vals = {
                        ...
                        'columns': [
                            {'name': v} for v in [
                                aml.move_id.ref,
                                format_date(
                                    self.env, aml.date_maturity or aml.date),
                                aml.journal_id.code,
                                aml.account_id.display_name,
                                format_date(
                                    self.env, aml.expected_pay_date)]
                        ] +
                        [
                            {'name': self.format_value(
                                sign * v, blank_if_zero=True),
                                'no_format': sign * v} for v in [
                                line['period'] == 6-i and line['amount']
                        ...
                        'title_hover': self._format_aml_name(
                            aml.name, aml.ref, aml.move_id.name),
                    }
                    lines.append(vals)
        return lines

    @api.model
    def _get_total_lines(self, total, line_id, sign):
        lines = []
    ...
    def _get_lines(self, options, line_id=None):
        ...
        if line_id and 'partner_' in line_id:
            # we only want to fetch data about this partner because
            # we are expanding a line
        ...
        results, total, amls = (
            self.env['report.account.report_agedpartnerbalance'
                     ].with_context(**context)._get_partner_move_lines(
                account_types, self._context['date_to'], 'posted', 30))
        lines += self._get_partner_lines(results, amls, sign, options)
        lines += self._get_total_lines(total, line_id, sign)
        return lines
